...
class WeiBoProfile(models.Model):
    # 1.外键 user_profile  OneToOneField()
    user_profile = models.OneToOneField(UserProfile, on_delete=models.CASCADE, null=True)
    # 2.微博id: wuid 字符类型 长度10
    wuid = models.CharField("微博uid", max_length=10)
    # 3.访问令牌: access_token 字符类型 长度32
    access_token = models.CharField("访问令牌", max_length=32)
    # 4.创建时间: created_time
    created_time = models.DateTimeField(auto_now_add=True)
    # 5.更新时间: updated_time
    updated_time = models.DateTimeField(auto_now=True)

    # 更改表名: users_weibo_profile
    class Meta:
